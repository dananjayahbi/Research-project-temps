...
class App:
    def __init__(self, master):
        ...
        # 🔥 NEW: Predicted Emotion Label (centered)
        self.predicted_emotion_label = ttk.Label(self.main_frame, text="Predicted Next Emotion: None", font=("Helvetica", 14))
        self.predicted_emotion_label.grid(row=2, column=0, pady=10)
        ...
        self.aggregator = EmotionAggregator(window_size=10, save_path="emotion_data.json")
    ...
    def update(self):
        ...
        if ret:
            ...
            if results:
                # Sum up emotions from multiple faces
                ...
                self.current_emotion_label.config(
                    text=f"Current Emotion: {current_emotion} ({confidence * 100:.2f}%)"
                )

                # Store emotions for LSTM/Transformer tracking
                self.aggregator.add_emotion(avg_emotion)

                # Get next predicted emotion from LSTM
                predicted_emotions = self.aggregator.predict_next_emotion()

                if predicted_emotions:
                    # Apply softmax to normalize predictions
                    pred_values = np.array(list(predicted_emotions.values()))
                    softmax_values = np.exp(pred_values) / np.sum(np.exp(pred_values))

                    # Get most likely next emotion
                    predicted_emotion = max(predicted_emotions, key=predicted_emotions.get)
                    pred_confidence = softmax_values[list(predicted_emotions.keys()).index(predicted_emotion)]

                    self.predicted_emotion_label.config(
                        text=f"Predicted Next Emotion: {predicted_emotion} ({pred_confidence * 100:.2f}%)"
                    )
                else:
                    self.predicted_emotion_label.config(text="Predicted Next Emotion: N/A")
            ...
            # Convert frame to RGB and update GUI
